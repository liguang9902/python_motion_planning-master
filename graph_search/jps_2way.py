...
class JPS_2way(AStar):
    ...
    def plan(self):
        ...
        # goal_A.current = self.goal
        # goal_B.current = self.start
        sum_t = 0.0
        while OPEN_A and OPEN_B:
            ...
            # goal_B = node_A
            # goal_A = node_B
            # x = [k for k in CLOSED_B if k in CLOSED_A]
            # print[x]
            # exists in CLOSED set
            if node_A not in CLOSED_A :
                
                if node_A in CLOSED_B: #and len(x) > 2:#and self.start not in CLOSED_B and self.goal not in CLOSED_A:
                    
                    CLOSED_A.append(node_A)
                    # CLOSED_B.append(node_B)
                    
                    costA, pathA =self.extractPathA(CLOSED_A)
                    costB, pathB =self.extractPathB1(CLOSED_B)
            ...
            if node_B not in CLOSED_B:             
                
                if node_B in CLOSED_A: #and len(x) > 2:#and self.start not in CLOSED_B and self.goal not in CLOSED_A: 
                
                    # CLOSED_A.append(node_A)
                    CLOSED_B.append(node_B)
                    
                    costA, pathA =self.extractPathA1(CLOSED_A)
    ...
    def jump1(self, node: Node, motion: Node):
        ...
        new_node = node + motion
        new_node.parent = node.current
        new_node.h = self.h(new_node, goal_B)
    ...
    def extractPathA(self, closed_set):
        ...
        cost = 0
        node = closed_set[-1]
        # node = closed_set[closed_set.index(x[0])]
        # node = node_B 
        path = [node.current]
        ...
        return cost, path
    def extractPathA1(self, closed_set):
        '''
        Extract the path based on the CLOSED set.
        ...
        cost = 0
        # node = closed_set[-1]
        node = closed_set[closed_set.index(node_B)]
        # node = closed_set[closed_set.index(x[0])]
        path = [node.current]
        while node != self.start:
            node_parent = closed_set[closed_set.index(Node(node.parent, None, None, None))]
    ...
    def extractPathB(self, closed_set):
        ...
        return cost, path
    def extractPathB1(self, closed_set):
        '''
        Extract the path based on the CLOSED set.
        ...
        # node = closed_set[closed_set.index(OPEN_B[OPEN_B.index(node_A)])]

        node = closed_set[closed_set.index(node_A)]
        # node = closed_set[closed_set.index(x[0])]
        path = [node.current]
        while node != self.goal:
            node_parent = closed_set[closed_set.index(Node(node.parent, None, None, None))]
    ...
    def cal_h(self, node: Node, goal: Node) -> float:
        dx = abs(goal.current[0] - node.current[0])                          #  Diagnol distance 
        dy = abs(goal.current[1] - node.current[1])
        min_xy = min(dx,dy)
        d = dx + dy + (math.sqrt(2) - 2) * min_xy  
